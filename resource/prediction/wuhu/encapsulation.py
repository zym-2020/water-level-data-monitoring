...
def wl_wuhu(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/anhui/getInfoByStationAndTime/芜湖' + \
        time_start_wl + time_end_wl
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_wuhu = requests.get(url_wl_anqing, verify=False).text
...
def wl_wuhu_test(time1, time2):
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/anhui/getInfoByStationAndTime/芜湖' + time1 + time2
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    if (len(error_correction) < 3):
        # 安庆实测水位数据
        wuhu_test = wl_wuhu_test(time1, time2)

    return tide_pre, warter_pre, wuhu_test
...
def correction(time_now):
    ...
    tide_pre, warter_pre, wuhu_test = main_def(
        time_start, time_end, time_pre_start, time_pre_end)
    # error_correction =  np.sum(((tide_pre + warter_pre) - wuhu_test), axis=0) / len(warter_pre)
    if (wuhu_test):
        error_correction = (tide_pre + warter_pre) - wuhu_test
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/wuhu/err.log"
    if len(sys.argv) != 3:
