...
def wl_tianshenggang(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_tianshenggang = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/天生港' + \
...
def wl_tianshenggang_test(time1, time2):
    url_wl_tianshenggang = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/天生港' + time1 + time2
    # print(url_wl_nanjing)
    requests.packages.urllib3.disable_warnings()
    res_tianshenggang = requests.get(url_wl_tianshenggang, verify=False).text
    ...
    while i < 24:
        if (j == len(json_dict_z_test)-1):
            tianshenggang_test.append(
                json_dict_z_test[j]['upstreamWaterLevel'])
            i = i + 1
            ...
            tianshenggang_test.append(
                json_dict_z_test[0]['upstreamWaterLevel'])
            i = i + 1
            ...
            tianshenggang_test.append(
                (json_dict_z_test[j-1]['upstreamWaterLevel'] + json_dict_z_test[j]['upstreamWaterLevel'])/2)
            i = i + 1
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    # 获取南京水位数据
    json_dict_z = wl_tianshenggang(time2)
    ...
    if (len(error_correction) < 3):
        # 南京实测水位数据
        tianshenggang_test = wl_tianshenggang_test(time1, time2)
...
def correction(time_now):
    ...
    tide_pre, warter_pre, jiangyin_test = main_def(
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/tianshenggang/err.log"
    if len(sys.argv) != 3:
