...
url_wusong = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/吴淞'
...
def wl_wusong():
    ...
    url_wl_wusong = url_wusong + time_start_wl + time_end_wl
    # print(url_wl_wusong)
    requests.packages.urllib3.disable_warnings()
    res_wusong = requests.get(url_wl_wusong, verify=False).text
    json_dict_z = json.loads(res_wusong)['data']
...
# 获得测试数据，一天
def wl_wusong_test(time1,time2):
    url_wl_wusong = url_wusong + time1 + time2
    # print(url_wl_wusong)
    requests.packages.urllib3.disable_warnings()
    res_wusong = requests.get(url_wl_wusong, verify=False).text
    json_dict_z_test = json.loads(res_wusong)['data']
    wusong_test = []
    ...
    while i < 24:
        if(j==len(json_dict_z_test)-1):
            wusong_test.append(json_dict_z_test[j]['upstreamWaterLevel'])
            i = i + 1
            ...
            wusong_test.append(json_dict_z_test[0]['upstreamWaterLevel'])
            i = i + 1
            ...
            wusong_test.append((json_dict_z_test[j-1]['upstreamWaterLevel'] + json_dict_z_test[j]['upstreamWaterLevel'])/2)
            i = i + 1
    # print(wusong_test)
    if((i-j-1) > 0):
...
def main_def(time_month,time_day,error_correction=[0]):
    ...
    # 获取南京水位数据
    json_dict_z = wl_wusong()
    # region 得到日均水位
    # 获得潮差和日均径流训练集
    mean_flow_3m, tide_range_3m, mean_wl_3m = get_tr_mf(json_dict_z)
    # 回归分析参数
    param_wusong = get_param(mean_flow_3m, tide_range_3m, mean_wl_3m)
    ...
    if(mean_flow and tide_range):
        warter_pre = water_pre_year(mean_flow, tide_range, param_wusong)
        # 预测日均水位修正
    ...
    if(len(error_correction)<3):
        ...
        wusong_test = wl_wusong_test(wltest1, wltest2)

    return tide_pre,warter_pre,wusong_test;
...
def correction(time_month,time_day):
    ...
    tide_pre, warter_pre, wusong_test = main_def(time_month, time_day)
    # error_correction =  np.sum(((tide_pre + warter_pre) - wusong_test), axis=0) / len(warter_pre)
    if(wusong_test):
        error_correction =  (tide_pre + warter_pre) - wusong_test
        return error_correction
    return [];

# 最终预测水位效果图
def plot_error(tide_pre,warter_pre,wusong_test,time):
    if(wusong_test==[]):
        return []
    plt.figure()
    ...
    plt.plot(wusong_test, marker='+', color='orange',label='true tide')
    plt.plot((tide_pre + warter_pre) - wusong_test, marker='.', color='red',label='error')
    plt.title(time + " wusong tide water level")
    plt.legend()
    ...
    plt.ylim(-2, 8)
    plt.show();
    error = (tide_pre + warter_pre) - wusong_test
    return error;
...
if __name__ == '__main__':
    ...
    # 最终误差结果
    error_final = plot_error(tide_pre,warter_pre,wusong_test,time_month+time_day)
    # 平均误差
