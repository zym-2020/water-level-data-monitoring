...
def wl_wusong(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/吴淞' + \
        time_start_wl + time_end_wl
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_wusong = requests.get(url_wl_anqing, verify=False).text
...
def wl_wusong_test(time1, time2):
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/吴淞' + time1 + time2
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_wusong = requests.get(url_wl_anqing, verify=False).text
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    if (len(error_correction) < 3):
        # 安庆实测水位数据
        wusong_test = wl_wusong_test(time1, time2)
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/wusong/err.log"
    if len(sys.argv) != 3:
