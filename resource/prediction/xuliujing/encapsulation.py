...
def wl_xuliujing(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/徐六泾' + \
        time_start_wl + time_end_wl
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_xuliujing = requests.get(url_wl_anqing, verify=False).text
...
def wl_xuliujing_test(time1, time2):
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/徐六泾' + time1 + time2
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_xuliujing = requests.get(url_wl_anqing, verify=False).text
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    # 获取徐六泾水位数据
    json_dict_z = wl_xuliujing(time2)
    ...
    if (len(error_correction) < 3):
        # 安庆实测水位数据
        xuliujing_test = wl_xuliujing_test(time1, time2)
...
def correction(time_now):
    ...
    tide_pre, warter_pre, xuliujing_test = main_def(
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/xuliujing/err.log"
    if len(sys.argv) != 3:
