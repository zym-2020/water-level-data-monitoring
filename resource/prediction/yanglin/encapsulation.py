...
def wl_yanglin(time_end_wl):
    time_start_wl = '/2023-03-23%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_yanglin = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/杨林' + \
...
def wl_yanglin_test(time1, time2):
    url_wl_yanglin = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/杨林' + time1 + time2
    # print(url_wl_yanglin)
    requests.packages.urllib3.disable_warnings()
    res_yanglin = requests.get(url_wl_yanglin, verify=False).text
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    if (len(error_correction) < 3):
        # 安庆实测水位数据
        yanglin_test = wl_yanglin_test(time1, time2)
...
def correction(time_now):
    ...
    tide_pre, warter_pre, yanglin_test = main_def(
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/yanglin/err.log"
    if len(sys.argv) != 3:
