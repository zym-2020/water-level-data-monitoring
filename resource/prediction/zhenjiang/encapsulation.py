...
def wl_zhenjiang(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_zhenjiang = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/镇江' + \
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    # 获取南京水位数据
    json_dict_z = wl_zhenjiang(time2)
    ...
    if (len(error_correction) < 3):
        # 南京实测水位数据
        jiangyin_test = wl_zhenjiang_test(time1, time2)
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/zhenjiang/err.log"
    if len(sys.argv) != 3:
