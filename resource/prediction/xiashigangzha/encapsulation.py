...
def wl_xiashigangzha(time_end_wl):
    time_start_wl = '/2023-03-25%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/夏仕港闸' + \
        time_start_wl + time_end_wl
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_xiashigangzha = requests.get(url_wl_anqing, verify=False).text
...
def wl_xiashigangzha_test(time1, time2):
    url_wl_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/夏仕港闸' + time1 + time2
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_xiashigangzha = requests.get(url_wl_anqing, verify=False).text
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    # 获取夏仕港闸水位数据
    json_dict_z = wl_xiashigangzha(time2)
    ...
    if (len(error_correction) < 3):
        # 安庆实测水位数据
        wusong_test = wl_xiashigangzha_test(time1, time2)
...
def correction(time_now):
    ...
    tide_pre, warter_pre, wusong_test = main_def(
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/xiashigangzha/err.log"
    if len(sys.argv) != 3:
