...
# 上下游径流和潮差数据时间长度
time_start = '/2023-03-25%2000:00:00'
# time_end = '/2023-05-20%2000:00:00'
time_end = '/2023-'+ str(mon) + '-' + str(mday) + '%2000:00:00'

# 水位调和分析数据时间长度
time_start_wl = '/2023-03-25%2009:00:00'
# time_end_wl = '/2023-05-20%2023:00:00'
time_end_wl = '/2023-'+ str(mon) + '-' + str(mday) + '%2023:00:00'

url_xiashigangzha = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/夏仕港闸'
...
# 几个月水位 2.20开始
def wl_xiashigangzha():

    url_wl_xiashigangzha = url_xiashigangzha + time_start_wl + time_end_wl
    # print(url_wl_xiashigangzha)
    requests.packages.urllib3.disable_warnings()
    res_xiashigangzha = requests.get(url_wl_xiashigangzha, verify=False).text
    json_dict_z = json.loads(res_xiashigangzha)['data']
...
def get_wl_eh(json_dict_z):
    ...
    k_f = 1
    Month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    for i in range(initial_month, 12):
        j = 1
        while j < Month[i - 1] + 1:
            ...
            while k < 24:
                ...
                if (t < len(json_dict_z)):
                    ...
                    if ((json_dict_z[t]['time'][6:7] == str(i)) and (json_dict_z[t]['time'][8:10] == j_str) and (
                        ...
                        t = t + 1
                    elif((json_dict_z[t+1]['time'][6:7] == str(i)) and (json_dict_z[t+1]['time'][8:10] == j_str) and (
...
# 获得测试数据，一天
def wl_xiashigangzha_test(time1,time2):
    url_wl_xiashigangzha = url_xiashigangzha + time1 + time2
    # print(url_wl_xiashigangzha)
    requests.packages.urllib3.disable_warnings()
    res_xiashigangzha = requests.get(url_wl_xiashigangzha, verify=False).text
    json_dict_z_test = json.loads(res_xiashigangzha)['data']
    xiashigangzha_test = []
    ...
    while i < 24:
        if(j==len(json_dict_z_test)-1):
            xiashigangzha_test.append(json_dict_z_test[j]['upstreamWaterLevel'])
            i = i + 1
            ...
            xiashigangzha_test.append(json_dict_z_test[0]['upstreamWaterLevel'])
            i = i + 1
            ...
            xiashigangzha_test.append((json_dict_z_test[j-1]['upstreamWaterLevel'] + json_dict_z_test[j]['upstreamWaterLevel'])/2)
            i = i + 1
    # print(xiashigangzha_test)
    if((i-j-1) > 0):
        print('%s 缺少%d项数据' %(json_dict_z_test[j]['time'][0:10],i-j-1))
    return xiashigangzha_test

# 主函数
def main_def(time_month,time_day,error_correction=[0]):

    # 获取南京水位数据
    json_dict_z = wl_xiashigangzha()
    # region 得到日均水位
    # 获得潮差和日均径流训练集
    mean_flow_3m, tide_range_3m, mean_wl_3m = get_tr_mf(json_dict_z)
    # 回归分析参数
    param_xiashigangzha = get_param(mean_flow_3m, tide_range_3m, mean_wl_3m)
    ...
    if(mean_flow and tide_range):
        warter_pre = water_pre_year(mean_flow, tide_range, param_xiashigangzha)
        # 预测日均水位修正
    ...
    if(len(error_correction)<3):
        ...
        xiashigangzha_test = wl_xiashigangzha_test(wltest1, wltest2)

    return tide_pre,warter_pre,xiashigangzha_test;
...
def correction(time_month,time_day):
    ...
    tide_pre, warter_pre, xiashigangzha_test = main_def(time_month, time_day)
    # error_correction =  np.sum(((tide_pre + warter_pre) - xiashigangzha_test), axis=0) / len(warter_pre)
    if(xiashigangzha_test):
        error_correction =  (tide_pre + warter_pre) - xiashigangzha_test
        return error_correction
    return [];

# 最终预测水位效果图
def plot_error(tide_pre,warter_pre,xiashigangzha_test,time):
    if(xiashigangzha_test==[]):
        return []
    plt.figure()
    ...
    plt.plot(xiashigangzha_test, marker='+', color='orange',label='true tide')
    plt.plot((tide_pre + warter_pre) - xiashigangzha_test, marker='.', color='red',label='error')
    plt.title(time + " xiashigangzha tide water level")
    plt.legend()
    ...
    plt.ylim(-2, 8)
    plt.show();
    error = (tide_pre + warter_pre) - xiashigangzha_test
    return error;
...
if __name__ == '__main__':
    ...
    wltest1 = '/2023-' + str(time_month) + '-' + str(time_day) + '%2000:00:00'
    wltest2 = '/2023-' + str(time_month) + '-' + str(time_day) + '%2023:00:00'
    # xiashigangzha_test 为真实潮位检测
    xiashigangzha_test = wl_xiashigangzha_test(wltest1, wltest2)

    # 最终误差结果
    error_final = plot_error(tide_pre,warter_pre,xiashigangzha_test,time_month+time_day)
    # 平均误差
