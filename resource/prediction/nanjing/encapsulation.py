...
def wl_nanjing(time_end_wl):
    ...
    url_wl_nanjing = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/%E5%8D%97%E4%BA%AC' + \
...
def plot_error(tide_pre, warter_pre, nanjing_test, time):
    if (nanjing_test == []):
        return []
    plt.figure()
    plt.plot(tide_pre + warter_pre, marker='o',
             color='blue', label='predicted tide')
    ...
    plt.plot((tide_pre + warter_pre) - nanjing_test,
             marker='.', color='red', label='error')
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/nanjing/err.log"
    if len(sys.argv) != 3:
