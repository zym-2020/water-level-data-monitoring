...
def wl_jiangyin(time_end_wl):
    time_start_wl = '/2023-02-20%2009:00:00'
    # time_end_wl = '/2023-05-20%2023:00:00'
    url_wl_jiangyin = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/江阴' + \
...
def wl_jiangyin_test(time1, time2):
    url_wl_jiangyin = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/江阴' + time1 + time2
    # print(url_wl_nanjing)
    requests.packages.urllib3.disable_warnings()
    res_jiangyin = requests.get(url_wl_jiangyin, verify=False).text
...
def main_def(time_start, time_end, time_pre_start, time_pre_end, error_correction=[0]):
    ...
    if (len(error_correction) < 3):
        # 南京实测水位数据
        jiangyin_test = wl_jiangyin_test(time1, time2)
...
if __name__ == '__main__':
    log_path = "E:/yangtze/resource/prediction/jiangyin/err.log"
    if len(sys.argv) != 3:
