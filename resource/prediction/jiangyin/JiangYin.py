...
url_jiangyin = 'https://geomodeling.njnu.edu.cn/waterLevel/jiangsu/getInfoByStationAndTime/江阴'
...
def wl_jiangyin():
    ...
    url_wl_jiangyin = url_jiangyin + time_start_wl + time_end_wl
    # print(url_wl_jiangyin)
    requests.packages.urllib3.disable_warnings()
    res_jiangyin = requests.get(url_wl_jiangyin, verify=False).text
    json_dict_z = json.loads(res_jiangyin)['data']
...
# 获得测试数据，一天
def wl_jiangyin_test(time1,time2):
    url_wl_jiangyin = url_jiangyin + time1 + time2
    # print(url_wl_jiangyin)
    requests.packages.urllib3.disable_warnings()
    res_jiangyin = requests.get(url_wl_jiangyin, verify=False).text
    json_dict_z_test = json.loads(res_jiangyin)['data']
    jiangyin_test = []
    ...
    while i < 24:
        if(j==len(json_dict_z_test)-1):
            jiangyin_test.append(json_dict_z_test[j]['upstreamWaterLevel'])
            i = i + 1
            ...
            jiangyin_test.append(json_dict_z_test[0]['upstreamWaterLevel'])
            i = i + 1
            ...
            jiangyin_test.append((json_dict_z_test[j-1]['upstreamWaterLevel'] + json_dict_z_test[j]['upstreamWaterLevel'])/2)
            i = i + 1
    # print(jiangyin_test)
    if((i-j-1) > 0):
        print('%s 缺少%d项数据' %(json_dict_z_test[j]['time'][0:10],i-j-1))
    return jiangyin_test

# 主函数
def main_def(time_month,time_day,error_correction=[0]):

    # 获取南京水位数据
    json_dict_z = wl_jiangyin()
    # region 得到日均水位
    # 获得潮差和日均径流训练集
    mean_flow_3m, tide_range_3m, mean_wl_3m = get_tr_mf(json_dict_z)
    # 回归分析参数
    param_jiangyin = get_param(mean_flow_3m, tide_range_3m, mean_wl_3m)
    ...
    if(mean_flow and tide_range):
        warter_pre = water_pre_year(mean_flow, tide_range, param_jiangyin)
        # 预测日均水位修正
    ...
    if(len(error_correction)<3):
        ...
        jiangyin_test = wl_jiangyin_test(wltest1, wltest2)

    return tide_pre,warter_pre,jiangyin_test;
...
def correction(time_month,time_day):
    ...
    tide_pre, warter_pre, jiangyin_test = main_def(time_month, time_day)
    # error_correction =  np.sum(((tide_pre + warter_pre) - jiangyin_test), axis=0) / len(warter_pre)
    if(jiangyin_test):
        error_correction =  (tide_pre + warter_pre) - jiangyin_test
        return error_correction
    return [];

# 最终预测水位效果图
def plot_error(tide_pre,warter_pre,jiangyin_test,time):
    if(jiangyin_test==[]):
        return []
    plt.figure()
    ...
    plt.plot(jiangyin_test, marker='+', color='orange',label='true tide')
    plt.plot((tide_pre + warter_pre) - jiangyin_test, marker='.', color='red',label='error')
    plt.title(time + " jiangyin tide water level")
    plt.legend()
    ...
    # plt.ylim(-2, 8)
    plt.show();
    error = (tide_pre + warter_pre) - jiangyin_test
    return error;
...
if __name__ == '__main__':
    ...
    wltest1 = '/2023-' + str(time_month) + '-' + str(time_day) + '%2000:00:00'
    wltest2 = '/2023-' + str(time_month) + '-' + str(time_day) + '%2023:00:00'
    # jiangyin_test 为真实潮位检测
    jiangyin_test = wl_jiangyin_test(wltest1, wltest2)

    # 最终误差结果
    error_final = plot_error(tide_pre,warter_pre,jiangyin_test,time_month+time_day)
    # 平均误差
