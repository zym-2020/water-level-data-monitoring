...
url_anqing = 'https://geomodeling.njnu.edu.cn/waterLevel/YangtzeDownstream/getInfoByStationAndTime/安庆'
...
def tideRange_flow(time1,time2,time_now):
    ...
    res_anqing = requests.get(url_flow_datong, verify=False).text
    json_dict = json.loads(res_anqing)['data']
...
def wl_anqing():
    ...
    url_wl_anqing = url_anqing + time_start_wl + time_end_wl
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
...
def wl_anqing_test(time1,time2):
    url_wl_anqing = url_anqing + time1 + time2
    # print(url_wl_anqing)
    requests.packages.urllib3.disable_warnings()
    res_anqing = requests.get(url_wl_anqing, verify=False).text
...
def main_def(time_month,time_day,error_correction=[0]):
    ...
    if(mean_flow and tide_range):
        water_pre = water_pre_year(mean_flow, tide_range, param_anqing)
        # 预测日均水位修正
    ...
    if(len(error_correction)<3):
        ...
        anqing_test = wl_anqing_test(wltest1, wltest2)
    return tide_pre,water_pre,anqing_test;
...
def correction(time_month,time_day):
    ...
    tide_pre, water_pre, anqing_test = main_def(time_month, time_day)
    # error_correction =  np.sum(((tide_pre + water_pre) - anqing_test), axis=0) / len(water_pre)
    if(anqing_test):
        error_correction =  (tide_pre + water_pre) - anqing_test
        return error_correction
    return [];

# 最终预测水位效果图
def plot_error(tide_pre,water_pre,anqing_test,time):
    if(anqing_test==[]):
        return []
    plt.figure()
    ...
    plt.plot(anqing_test, marker='+', color='orange',label='true tide')
    plt.plot((tide_pre + water_pre) - anqing_test, marker='.', color='red',label='error')
    plt.title(time + " anqing tide water level")
    plt.legend()
    ...
    # plt.ylim(-2, 8)
    plt.show();
    error = (tide_pre + water_pre) - anqing_test
    return error;
...
if __name__ == '__main__':
    ...
    # 最终误差结果
    error_final = plot_error(tide_pre,water_pre,anqing_test,time_month+time_day)
    # 平均误差
